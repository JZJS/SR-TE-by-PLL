...
class PrunedLandmarkLabeling:
    ...
    def is_loop_free(self, path):
        return len(path) == len(set(path))

    def get_best_landmark_set(self, landmarks, src, dest, bw, K):
        ...
        for _ in range(K):
            ...
            for landmark in available_landmarks:
                ...
                valid_paths = [path for path in paths if self.is_loop_free(path)]  # Filter out paths with loops

                if not valid_paths:  # If there are no valid paths for the current landmark, skip it
                    continue

                total_score_for_landmark = 0
                for path in valid_paths:
                    ...
                    total_score_for_landmark += score

                # If only one landmark is being considered, increase its total_score
                if K == 1:
                    total_score_for_landmark *= 2

                if total_score_for_landmark < min_score:
                    min_score = total_score_for_landmark
                    best_landmark = landmark
                    best_path = valid_paths[-1]  # This will be the last valid path for the best landmark
            ...
            if best_landmark and best_path:
                ...
                self.update_bandwidth([best_path], bw/K)
