...
class Dijkstra:
    ...
    def query(self, src, dest):
        try:
            # 获取所有最短路径
            paths = list(nx.all_shortest_paths(self.graph, src, dest, weight='weight'))
            lengths = [nx.dijkstra_path_length(self.graph, src, dest, weight='weight') for path in paths]
    ...
    def update_bandwidth(self, paths, flow_size):
        num_paths = len(paths)
        flow_per_path = flow_size / num_paths
        for path in paths:
            for i in range(len(path) - 1):
                self.graph[path[i]][path[i+1]]['bandwidth'] -= flow_per_path
...
def main():
    ...
    for src, dest, bw in D:
        ...
        lengths, paths = dijkstra.query(src, dest)
        for i, path in enumerate(paths):
            print(f'Shortest path length from {src} to {dest} (Path {i+1}):', lengths[i])
            print(f'Shortest path from {src} to {dest} (Path {i+1}):', path)
        dijkstra.update_bandwidth(paths, bw)
