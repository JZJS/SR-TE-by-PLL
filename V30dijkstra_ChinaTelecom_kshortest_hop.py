...
class Kshortest:
    ...
    def k_shortest_paths_query(self, src, dest, k=3):
        paths = self.k_shortest_paths(src, dest, k, weight='weight')
        lengths = [sum(self.graph[path[i]][path[i+1]]['weight'] for i in range(len(path) - 1)) for path in paths]
    ...
    def update_bandwidth(self, paths, flow_size):
        for path in paths:
            for i in range(len(path) - 1):
                self.graph[path[i]][path[i+1]]['bandwidth'] -= flow_size / len(paths)
...
def main():
    ...
    # Simulate some flows and update the network bandwidth using k shortest paths
    for src, dest, bw in D:
        lengths, paths = kshortest.k_shortest_paths_query(src, dest, k=3)
        for i, path in enumerate(paths):
            print(f'Path {i+1} length from {src} to {dest}:', lengths[i])
            print(f'Path {i+1} from {src} to {dest}:', path)
        kshortest.update_bandwidth(paths, bw)
        all_paths.extend(paths)  # 将路径添加到all_paths列表中

    # Query the link with the maximum utilization
    max_link, max_utilization = kshortest.get_max_utilization_link()
    print('Link with maximum utilization:', max_link)
    print('Maximum utilization:', "{:.2%}".format(max_utilization))

    # 计算并打印平均跳数
    avg_hop_count = kshortest.average_hop_count(all_paths)
